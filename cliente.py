...
class Chat:
    ...
    def janela(self):
        ...
        self.root.title(f'Chat - {self.sala}')

        self.caixa_texto = Text(self.root)
        self.caixa_texto.place(relx=0.06 , rely=0.04, width=700, height=600)

        self.envia_mensagem = Entry(self.root)
        self.envia_mensagem.place(relx=0.06, rely=0.84, width=600, height=35)

        self.btn_enviar = Button(self.root, text='Enviar', command=self.enviarMensagem)
        self.btn_enviar.place(relx=0.81, rely=0.84, width=100, height=35)
